...
import os, fnmatch, multiprocessing, copy
import numpy as np
...
def basicProc(tr, f, typ):
    ...
    if config.cfg['Streams']['Resample'][0] == True:
        tr.resample(config.cfg['Streams']['Resample'][1], window='hann', no_filter=True, strict_length=False)
...
def commonMetric(st, raw, q):
    ...
    # Apply pre-filter
    if config.cfg['Streams']['Pre Filter'][0] == 0 and config.cfg['Streams']['Pre Filter'][1] == 0:
         pass
    else:
         st.filter("bandpass", freqmin=config.cfg['Streams']['Pre Filter'][0],\
                   freqmax=config.cfg['Streams']['Pre Filter'][1])
...
def checkD(stream):
    ...
    """
    #Make upper the list
    config.cfg['Streams']['Quality Control'] = [each_method.upper() for each_method in config.cfg['Streams']['Quality Control']]
    ...
    if 'SNR' in config.cfg['Streams']['Quality Control']:

        Qdec = checkSNR(config.st.select(station=stream[1], channel=stream[3]).copy()[0],\
                        Qdec, 'KURT', 5, 5)

    if 'TIME' in config.cfg['Streams']['Quality Control']:

        Qdec = checkTIME(config.st.select(station=stream[1], channel=stream[3]).copy()[0],\
                         Qdec, 'KURT', 3)
    return Qdec
...
def clean():
    ...
    #In case of remain gaps left do merge and interpolate the missing values
    ...
    # Supported tests.
    quality_tests_list = ['SNR', 'TIME', 'CLIP']

    # Check if we have test names in the list.
    if any(value in quality_tests_list for value in config.cfg['Streams']['Quality Control']):
         ...
         for check in ['CLIP', 'SNR']:

             if check == 'CLIP' and 'CLIP' in config.cfg['Streams']['Quality Control'] and config.cfg['Plotting']['Plots']==True:

                 try:
                     config.logger.info('Plot CLIP results')
                     CLIPPlot(txtpath, copy.deepcopy(streams), config.st.copy(),\
                              copy.deepcopy(config.Qual))
                 except:
                     config.logger.warning('Error in CLIP plot')      


             if check == 'SNR' and 'SNR' in config.cfg['Streams']['Quality Control'] and config.cfg['Plotting']['Plots']==True:

                 try:
                     config.logger.info('Plot SNR results')
                     temp_st = config.st.copy()
                     SNRPlot(txtpath, copy.deepcopy(streams),\
                             temp_st.filter("bandpass", freqmin=1, freqmax=25), copy.deepcopy(config.Qual))
                 except:
                     config.logger.warning('Error in SNR plot')

         # remove from streams and results problematic cases
         index_with_issue = []

         for i in range(len(config.Qual)):
             if 'SNR' in config.cfg['Streams']['Quality Control']:
                  if config.Qual[i]['SNR'][0] == False:
                      index_with_issue.append(i)
                      config.logger.info('Trace ' + '{}.{}.{}.{}'.format(streams[i][0], streams[i][1],\
                                         streams[i][2], streams[i][3]) + ' removed from SNR' + ' test')

             if 'CLIP' in config.cfg['Streams']['Quality Control']:
                  if config.Qual[i]['CLIP'][0] == False:
                      index_with_issue.append(i)
                      config.logger.info('Trace ' + '{}.{}.{}.{}'.format(streams[i][0], streams[i][1],\
                                         streams[i][2], streams[i][3]) + ' removed from CLIP' + ' test')

         # Remove duplicates
         index_with_issue = list(set(index_with_issue))

         streams = [streams[i] for i in range(len(streams)) if i not in index_with_issue]
         config.Qual = [config.Qual[i] for i in range(len(config.Qual)) if i not in index_with_issue]

         temp_st = Stream()
         # Remove from stream
         for i in streams:
             temp_st.append(config.st.select(network = i[0], station = i[1], channel = i[3])[0].copy())
         config.st = temp_st         

 
         if 'TIME' in config.cfg['Streams']['Quality Control'] and config.cfg['Plotting']['Plots']==True:
             try:
                 config.logger.info('Plot TIME results')
                 temp_st = config.st.copy()
                 TIMEPlot(txtpath, copy.deepcopy(streams), temp_st.filter("bandpass", freqmin=1, freqmax=25),\
                          copy.deepcopy(config.Qual))
             except:
         ...
         for i in range(len(config.Qual)):
             if 'TIME' in config.cfg['Streams']['Quality Control']:
                  if config.Qual[i]['TIME'][0] == False:
                      index_with_issue.append(i)
                      config.logger.info('Trace ' + '{}.{}.{}.{}'.format(streams[i][0], streams[i][1],\
                                         streams[i][2], streams[i][3]) + ' removed from TIME' + ' test')
