...
def monitor(cfg, log):
    ...
    # Get the traces duration and create starttime and endtime
    traces_dur = np.abs(cfg['Streams']['Duration'][1] - cfg['Streams']['Duration'][0])

    endtime = UTCDateTime.now() - math.ceil(traces_dur) - 2 - cfg['Monitor']['Playback']
    
    starttime = endtime - math.ceil(traces_dur) - cfg['Monitor']['Range']
