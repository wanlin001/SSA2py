...
import shapefile
import cartopy.mpl.geoaxes, math, os 
...
def MaxBrightTimeStep_(brpath, brpathboot, evla, evlo, evdepth, time, inv, stations_used,\
    ...
    if (min_lon is None) and (min_lat is None) and (max_lon is None) and (max_lat is None):
        ...
        min_lon = evlo-kilometer2degrees(maxgrid)/2 - 0.02
        min_lat = evla-kilometer2degrees(maxgrid)/2 - 0.02
        max_lon = evlo+kilometer2degrees(maxgrid)/2 + 0.02
        max_lat = evla+kilometer2degrees(maxgrid)/2 + 0.02
    ...
    ## Add the legend
        ...
        if Test=='MAIN' or Test=='ARF':
            from matplotlib.offsetbox import (AnchoredOffsetbox,
                                  TextArea, VPacker)
            box1 = TextArea("SSA Results", textprops=dict(color="k", fontsize='x-large', fontweight='bold'))

            if Test=='MAIN':
                box1 = TextArea("SSA Results", textprops=dict(color="k", fontsize='x-large', fontweight='bold'))
            if Test=='ARF':
                box1 = TextArea("Array Response Function", textprops=dict(color="k", fontsize='x-large', fontweight='bold'))
            ...
            box = VPacker(children=[box1, box3, box4, box5],
                  align="center",
                  pad=3, sep=5)
    ...
    fig.suptitle('Maximum Brightness Per Time Step', fontsize=20, fontweight='bold')
